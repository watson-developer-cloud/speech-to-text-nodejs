...
export function Keywords(props) {
    ...
    const list = props.keywords.map(k => (<li key={k}><b>{k}</b>: {renderSpottings(spotted[k])}</li>));
    ...
    return (
        <div>
            ...
            <p class="base--p base--p_light">Format for each spotted keyword is start time-end time (% confidence).</p>
        </div>
    );
}
