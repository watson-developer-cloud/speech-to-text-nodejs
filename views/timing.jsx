...
const Word = function(props){
    ...
    return (
        <span className="arrow-box-container" onClick={click} onMouseEnter={props.onMouseEnter} onMouseLeave={props.onMouseLeave}>
            ...
            <ArrowBox direction="top" show={props.showDetails} color={Colors.purple_50}>
                <div style={{color: 'white'}}>
                    ...
                    <ul className="base--ul">
                        {props.alternatives.map(w => (<li key={w.word} className="base--li">{w.word}: {Math.round(w.confidence * 1000)/10}%</li>))}
                    </ul>
                </div>
            </ArrowBox>
            {' '}
            ...
            const results = this.props.messages.map(msg => {
                ...
                return msg.results.map((result, i) => (
                    <div key={msg.result_index + i}>
                        {(result.word_alternatives || []).map(alt => <Word {...alt}
                                                                           key={alt.start_time}
